    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 60, gap: 8 }}>
          ...
          right={
            <View style={[styles.segWrap, { borderColor: colors.border, backgroundColor: colors.surface }]}>
              {(['dark', 'light'] as const).map((m) => {
                const active = settings.mode === m && !settings.highContrast;
                return (
                  <Pressable
                    key={m}
                    onPress={() => setMode(m)}
        ...
        <View style={[styles.sliderCard, { borderColor: colors.border, backgroundColor: colors.surface }]}>
          ...
          <Text style={[styles.rowDesc, { color: colors.textSecondary }]}>{settings.ttsRate.toFixed(2)}×</Text>
          <Slider
            minimumValue={0.5}
            maximumValue={1.6}
            step={0.05}
            value={settings.ttsRate}
            onValueChange={(v) => update({ ttsRate: Number(v.toFixed(2)) })}
        ...
        <View style={[styles.sliderCard, { borderColor: colors.border, backgroundColor: colors.surface }]}>
          ...
          <Slider
            ...
            onValueChange={(v) => update({ ttsPitch: Number(v.toFixed(2)) })}
            minimumTrackTintColor={colors.primary}
            maximumTrackTintColor={colors.border}
            thumbTintColor={colors.primary}
        ...
        <Pressable
          onPress={() => speak('InkVoice is ready. Write naturally on the canvas and I will read your words aloud.', { rate: settings.ttsRate, pitch: settings.ttsPitch })}
          accessible accessibilityRole="button" accessibilityLabel="Test voice"
        ...
        <Pressable
          ...
          accessible accessibilityRole="button" accessibilityLabel="Clear saved history"
        ...
        <View style={[styles.sliderCard, { borderColor: colors.border, backgroundColor: colors.surface }]}>
          <Text style={[styles.rowTitle, { color: colors.textPrimary }]}>Gemini 3 Flash (cloud)</Text>
          <Text style={[styles.rowDesc, { color: colors.textSecondary }]}>
            High-accuracy handwriting OCR. To plug in a custom on-device model (TensorFlow Lite / ONNX), replace the
            `recognizeImage` call in `src/services/recognition.ts` with your local inference adapter.
          </Text>
        </View>
      </ScrollView>
    </SafeAreaView>
